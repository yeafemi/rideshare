  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("routes")
        .select(
          "*, profiles:driver_id(full_name, phone, verified, photo_url, work_email, trust_score, badges, school_name, cars(*))",
        )
        .eq("id", routeId)
        .maybeSingle();
      setRoute(data);
            ...
            <Card className="overflow-hidden border-none shadow-2xl bg-background/60 backdrop-blur-xl border border-primary/10">
              ...
              {isOwner && (
                <div className="p-6 bg-destructive/10 border-b border-destructive/20">
                  <div className="flex items-center gap-3 text-destructive font-black text-xs uppercase tracking-widest">
              ...
              <div className="flex items-start gap-4">
                <div className="relative">
                  ...
                  {route.profiles?.verified && (
                    <div className="absolute -bottom-1 -right-1 bg-success text-white rounded-full p-1 shadow-lg">
